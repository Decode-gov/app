...
export default function DominiosPage() {
          ...
          <CardContent className="space-y-4 pt-6">
            ...
            <div className="space-y-3">
              {Array.from({ length: 5 }, (_, i) => i).map((key) => (
                <div key={key} className="h-16 w-full bg-muted rounded" />
              ))}
            </div>
          </CardContent>
        </Card>
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-destructive">Domínios de dados</h1>
          <p className="text-muted-foreground mt-2">Erro ao carregar domínios de dados</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="group hover:shadow-lg transition-all duration-300">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Total de domínios cadastrados</p>
          </CardContent>
        </Card>
      ...
      <Card className="bg-card/80 backdrop-blur-sm border-border/60 shadow-lg">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Lista de todos os domínios de dados cadastrados</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <DominiosDataTable columns={columns} data={comunidades} parentOptions={parentOptions} />
        </CardContent>
      </Card>
