/**
 * Generated by orval v8.11.0 🍺
 * Do not edit manually.
 * API de Gestão de dados governamentais - DECODE-GOV
 * API completa para gestão de dados governamentais.
 * OpenAPI spec version: 1.0.0
 */
import { z as zod } from "zod";

export const getPoliticasInternas200DataItemIdRegExp = new RegExp(
  "^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$",
);

export const getPoliticasInternas200DataItemDominioDadosIdOneRegExp =
  new RegExp(
    "^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$",
  );

export const GetPoliticasInternas200 = zod.object({
  message: zod.string(),
  data: zod.array(
    zod.object({
      id: zod.uuid().regex(getPoliticasInternas200DataItemIdRegExp).optional(),
      nome: zod.string().min(1).optional(),
      descricao: zod.string().min(1).optional(),
      categoria: zod.string().min(1).optional(),
      objetivo: zod.string().min(1).optional(),
      escopo: zod.string().min(1).optional(),
      dominioDadosId: zod
        .union([
          zod
            .uuid()
            .regex(getPoliticasInternas200DataItemDominioDadosIdOneRegExp),
          zod.literal(null).nullable(),
        ])
        .optional(),
      responsavel: zod.string().min(1).optional(),
      dataCriacao: zod.iso.datetime({ offset: true }),
      dataInicioVigencia: zod.iso.datetime({ offset: true }),
      dataTermino: zod
        .union([
          zod.iso.datetime({ offset: true }),
          zod.literal(null).nullable(),
        ])
        .optional(),
      status: zod.enum(["Em_elaboracao", "Vigente", "Revogada"]),
      versao: zod.string().min(1),
      anexosUrl: zod
        .union([zod.string(), zod.literal(null).nullable()])
        .optional(),
      relacionamento: zod
        .union([zod.string(), zod.literal(null).nullable()])
        .optional(),
      observacoes: zod
        .union([zod.string(), zod.literal(null).nullable()])
        .optional(),
    }),
  ),
});

export type GetPoliticasInternas200 = zod.input<typeof GetPoliticasInternas200>;
export type GetPoliticasInternas200Output = zod.output<
  typeof GetPoliticasInternas200
>;
