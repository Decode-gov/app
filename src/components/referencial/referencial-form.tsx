...
import {
  usePostListasClassificacao,
  usePutListasClassificacaoId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing
              ? "Atualize os dados do referencial de classificação."
              : "Preencha os dados para criar um novo referencial."}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Descrição do referencial de classificação"
                      rows={4}
                      {...field}
                    />
                  </FormControl>
                  <FormDescription>
                    Detalhes sobre os critérios e aplicação desta classificação (máx. 2000
                    caracteres)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                disabled={createReferencial.isPending || updateReferencial.isPending}
              >
                {createReferencial.isPending || updateReferencial.isPending
                  ? "Salvando..."
                  : isEditing
                    ? "Atualizar"
                    : "Criar"}
      ...
      <PoliticaInternaForm open={isPoliticaFormOpen} onOpenChange={setIsPoliticaFormOpen} />
    </Dialog>
  );
}
