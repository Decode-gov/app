...
import type { GetPoliticasInternas200DataItem } from "@/api/generated/model";
...
const statusBadgeMap: Record<
  PoliticaInternaResponse["status"],
...
export const createColumns = ({
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <Badge variant={badgeInfo?.variant || "outline"}>{badgeInfo?.label || status}</Badge>;
    },
    filterFn: (row, id, value) => {
      return value.includes(row.getValue(id));
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="text-muted-foreground">{new Date(data).toLocaleDateString("pt-BR")}</div>
      );
    },
  },
  {
