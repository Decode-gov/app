      ...
      <AlertDialogContent className="bg-background/95 backdrop-blur-sm border-border/60">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-muted-foreground space-y-2">
            <span>
              Tem certeza de que deseja excluir a política{" "}
              <strong>&quot;{politica.nome}&quot;</strong>?
            </span>
            <div className="mt-4 p-4 bg-muted/30 rounded-lg space-y-2 text-sm">
              <div>
                <strong>ID:</strong> <span>{politica.id}</span>
              </div>
              <div>
                <strong>Categoria:</strong> <span>{politica.categoria}</span>
              </div>
              <div>
                ...
                <span>
                  {politica.status === "Vigente"
                    ? "Vigente"
                    : politica.status === "Em_elaboracao"
                      ? "Em Elaboração"
                      : politica.status === "Revogada"
                        ? "Revogada"
                        : politica.status}
                </span>
              </div>
              <div>
                <strong>Responsável:</strong> <span>{politica.responsavel}</span>
              </div>
              <div>
                <strong>Versão:</strong> <span>{politica.versao}</span>
              </div>
              <div>
                <strong>Criada em:</strong>{" "}
                <span>{format(politica.dataCriacao, "dd/MM/yyyy", { locale: ptBR })}</span>
              </div>
            </div>
